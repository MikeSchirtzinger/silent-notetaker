...
pub enum TimestampMode {
    /// Elapsed since recording start, `mm:ss` (the default).
    #[default]
    Elapsed,
    /// Wall-clock time, en-US 12-hour `h:mm AM/PM`.
    Clock,
    /// Relative age, `Ns ago` / `Nm ago` / `Nh ago`.
    Ago,
}
...
impl TimestampMode {
    ...
    /// `TIME_FORMATS = ['elapsed', 'clock', 'ago']`.
    pub const CYCLE: [TimestampMode; 3] = [
        TimestampMode::Elapsed,
        TimestampMode::Clock,
        TimestampMode::Ago,
    ];

    /// The next mode in the cycle (wraps), reproducing `cycleTimeFormat`'s
    /// `(i + 1) % TIME_FORMATS.length`. An unknown current mode restarts the
    /// cycle at `elapsed` — the JS `indexOf(...)` returns `-1`, and
    /// `(-1 + 1) % 3 == 0`, i.e. `elapsed`.
    #[must_use]
    pub fn next(self) -> TimestampMode {
        let idx = Self::CYCLE.iter().position(|&m| m == self);
        // `position` → `None` mirrors JS `indexOf == -1`; `(−1 + 1) % 3 == 0`.
        let next = match idx {
            Some(i) => (i + 1) % Self::CYCLE.len(),
            None => 0,
        };
        Self::CYCLE[next]
    }
    ...
    /// (`tsFormatBtn.textContent = fmt`).
    #[must_use]
    pub fn label(self) -> &'static str {
        match self {
            TimestampMode::Elapsed => "elapsed",
            TimestampMode::Clock => "clock",
            TimestampMode::Ago => "ago",
        }
    }
}
