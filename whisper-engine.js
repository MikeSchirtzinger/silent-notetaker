...
/**
 * The executor worker source: loads the transformers.js ASR pipeline and runs
 * `transcriber(audio)` per chunk, replying with the RAW decoded text. No policy
 * (VAD / hallucination / dedup are Rust). The main thread posts `{ type:
 * 'transcribe', chunk, audio }`; the worker replies `{ type: 'decoded', chunk,
 * text }`. Mirrors the model-load lifecycle messages (status/progress/ready/error)
 * the index.html UI already renders.
 */
const EXECUTOR_WORKER_SRC = `
